...
> Дата: 2026-09-24 · Итог: **PASS** (E2E T041 пройден)
...
Проверено в браузере на десктопе и на мобильной ширине 375 px (горизонтального скролла нет):

- ✅ Список из 10 машин, поиск «камри» → 1 результат, «ферра» → пустое состояние, кнопка сброса работает.
- ✅ Переход на `/cars/bmw-3-series`, хлебные крошки, переключение фото, подпись автора и лицензии.
- ✅ Три группы характеристик, история марки, кнопки auto.ru и Авито (`target=_blank`, `rel=noopener noreferrer`).
- ✅ `/cars/net-takoy` → «Автомобиль не найден».
- ⏳ AC-1.6 (бэкенд выключен → ошибка и «Повторить») — проверить вручную.

Найдено и исправлено при E2E:

1. **Шрифт с засечками вместо системного.** Стили темы Astryx действуют только внутри `data-astryx-theme`, поэтому приложение обёрнуто в `<Theme theme={neutralTheme}>` (`main.tsx`), как в официальном example-vite.
2. **Фон обрывался под коротким контентом.** Убран `height="auto"` у `AppShell`, по умолчанию стоит `fill` (на весь экран).
